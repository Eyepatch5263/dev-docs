...
export async function searchTerms(query: string): Promise<SearchResult> {
  ...
  const isElasticsearchConfigured = !!(
    process.env.ELASTICSEARCH_URL && process.env.ELASTICSEARCH_PROD_API_KEY
  );
  ...
  try {
    ...
    const terms = hits.map((hit: any) =>
      normalizeTermFromES(hit._source, hit._id),
    );
...
export async function getTermBySlug(
  ...
  if (!isElasticsearchConfigured) {
    const { getTermBySlug: localGetTerm } = await import(
      "../../data/sample-terms"
    );
...
export async function getRelatedTerms(
  ...
  if (shouldUseCache()) {
    ...
    console.log(
      `Production mode: Skipping cache for related terms: "${term.slug}"`,
    );
  }
  ...
  if (!isElasticsearchConfigured) {
    ...
    const { getRelatedTerms: localGetRelated } = await import(
      "../../data/sample-terms"
    );
  ...
  try {
    ...
    const response = await client.search({
      ...
      query: {
        bool: {
          ...
          must_not: [{ match_phrase: { term: term.term } }],
          minimum_should_match: 1,
        },
      },
      size: limit,
    ...
    const hits = response.hits.hits;
    const related = hits.map((hit: any) =>
      normalizeTermFromES(hit._source, hit._id),
    );
    ...
    console.error(
      "Elasticsearch related terms failed, using local fallback:",
      error,
    );
    const { getRelatedTerms: localGetRelated } = await import(
      "../../data/sample-terms"
