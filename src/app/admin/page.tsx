...
export default function AdminDashboard() {
  ...
  const isAdmin =
    !reviewQuery.error || (reviewQuery.error as Error).message !== "FORBIDDEN";
  const loading =
    status === "loading" ||
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          >
            <TabsList className="flex justify-between w-full mb-6">
              {(["review", "approved", "rejected", "all"] as TabType[]).map(
                (tab) => (
                  <TabsTrigger className="capitalize" key={tab} value={tab}>
                    {tab}
                    {documents[tab].length > 0 && (
                      <span
                        className={`md:ml-2 px-2 py-0.5 hidden md:block  text-white text-xs rounded-full ${adminBgColor[tab]}`}
                      >
