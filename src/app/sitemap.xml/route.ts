...
export async function GET() {
    ...
    const urlEntries = docs.map((doc) => {
        ...
        // Normalize siteUrl and slug to avoid duplicate slashes without touching the protocol
        const normalizedSite = siteUrl.replace(/\/+$/, "");
        const normalizedSlug = String(doc.slug).replace(/^\/+/, "");
        const loc = `${normalizedSite}/${normalizedSlug}`;
