...
import { Search, ArrowRight, Star, Clock, FileText, Lightbulb, Link2, ChevronRight, Cpu, Database, Globe, Layers, Terminal, Container } from 'lucide-react';
...
const featuredTerm = {
    ...
    date: 'Featured',
    definition: 'In distributed systems, you can only guarantee two out of the three: Consistency, Availability, and Partition Tolerance. The theorem helps in designing reliable distributed systems by understanding these trade-offs.',
    relatedTerms: ['PACELC', 'Consistency', 'Availability', 'Partition Tolerance', '+ 6 more'],
};

const recentTerms = [
    { term: 'ACID Properties', category: 'Database', slug: 'acid-properties' },
    { term: 'Eventual Consistency', category: 'Distributed Systems', slug: 'eventual-consistency' },
...
const stats = [
    { icon: FileText, value: '2000+', label: 'Terms', description: 'Curated engineering concepts across multiple domains' },
    { icon: Lightbulb, value: 'Clear', label: 'Explanations', description: 'Simple, accurate and practical explanations with examples' },
    { icon: Link2, value: 'Related', label: 'Concepts', description: 'Discover connected topics and learn in context' },
];
...
function OrbitingTerm({ label, color, x, y }: { label: string; color: string; x: number; y: number }) {
    return (
        <div
        ...
        <section className="relative w-full h-screen flex flex-col items-center overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-500">
            ...
            <div className="absolute inset-0 hidden lg:block pointer-events-none z-10">
                {positionedTerms.map((term, i) => (
                    <OrbitingTerm key={i} label={term.label} color={term.color} x={term.x} y={term.y} />
                ))}
            </div>
