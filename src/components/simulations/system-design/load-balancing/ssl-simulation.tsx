...
export default function SslSimulation() {
  ...
  return (
    <div className="w-full my-8 p-6 rounded-2xl border border-zinc-800/80 bg-zinc-950/85 backdrop-blur-md shadow-2xl flex flex-col gap-6 select-none text-zinc-100">
      ...
      <div className="flex bg-zinc-950/60 p-1.5 rounded-xl border border-zinc-850 self-start">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all border cursor-pointer ${
              strategy === mode
                ...
                : "bg-transparent text-zinc-400 border-transparent hover:text-zinc-200 disabled:opacity-50 disabled:cursor-not-allowed"
            }`}
          >
            {mode === "termination"
              ? "SSL Termination"
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          {/* Canvas Box */}
          <div className="relative h-[500px] w-full rounded-xl border border-zinc-800 bg-black/60 overflow-hidden flex items-center justify-around px-8">
            
            {/* Grid Pattern */}
            <div className="absolute inset-0 bg-[linear-gradient(to_right,#18181b_1px,transparent_1px),linear-gradient(to_bottom,#18181b_1px,transparent_1px)] bg-[size:16px_16px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_100%)] opacity-30 pointer-events-none" />

            {/* Left Node: Client */}
            <div className="flex flex-col items-center gap-3 z-10">
              <div className="w-14 h-14 rounded-full bg-zinc-900 border-2 border-blue-500/80 flex items-center justify-center shadow-lg shadow-blue-500/10">
                <User className="w-6 h-6 text-blue-400" />
              </div>
              <div className="text-center">
                <span className="text-[10px] font-bold text-zinc-400 tracking-wider block uppercase">Client</span>
                <span className="text-[9px] text-zinc-500 block">IP: 198.51.100.12</span>
              </div>
            </div>

            {/* Link wire 1 (Client to LB) */}
            <div className="flex-1 h-0.5 bg-zinc-800 relative mx-4">
            ...
            <div className="flex flex-col items-center gap-3 z-10 w-[140px]">
              <div className={`w-16 h-16 rounded-2xl bg-zinc-900 border-2 flex flex-col items-center justify-center shadow-lg transition-all ${
                activePacket?.phase === "processing-lb" 
                  ? "border-purple-400 bg-purple-950/10 shadow-purple-500/10 scale-105" 
                  : "border-zinc-700"
              }`}>
                <Activity className={`w-6 h-6 ${activePacket?.phase === "processing-lb" ? "text-purple-400 animate-pulse" : "text-zinc-400"}`} />
                {activePacket?.phase === "processing-lb" && (
                  <span className="text-[7.5px] font-extrabold uppercase text-purple-400 px-1 bg-purple-900/30 border border-purple-500/30 rounded mt-0.5 animate-pulse">
                    Decrypting
                  </span>
                )}
              </div>
              <div className="text-center w-full">
                <span className="text-[10px] font-bold text-zinc-400 tracking-wider block uppercase">Load Balancer</span>
                
                {/* LB CPU Gauge */}
                <div className="mt-1.5 px-3">
                  <div className="flex justify-between text-[8px] text-zinc-500 font-mono mb-0.5">
                    <span>CPU:</span>
                    <span className={lbCpu > 40 ? "text-red-400 font-bold" : "text-zinc-400"}>{Math.round(lbCpu)}%</span>
                  </div>
                  <div className="w-full bg-zinc-950 h-1 rounded-full overflow-hidden border border-zinc-900">
                    <div 
                      className={`h-full transition-all duration-300 ${
                        lbCpu > 60 ? "bg-red-500" : lbCpu > 30 ? "bg-purple-500" : "bg-zinc-700"
                      }`} 
                      style={{ width: `${lbCpu}%` }} 
                    />
                  </div>
            ...
            <div className="flex flex-col items-center gap-3 z-10 w-[140px]">
              <div className={`w-14 h-14 rounded-full bg-zinc-900 border-2 flex items-center justify-center shadow-lg transition-all ${
                activePacket?.phase === "processing-server" 
                  ? "border-emerald-400 bg-emerald-950/10 shadow-emerald-500/10 scale-105" 
                  : "border-zinc-800"
              }`}>
                <Server className={`w-5 h-5 ${activePacket?.phase === "processing-server" ? "text-emerald-400 animate-pulse" : "text-zinc-500"}`} />
              </div>
              <div className="text-center w-full">
                <span className="text-[10px] font-bold text-zinc-400 tracking-wider block uppercase">App Server</span>
                
                {/* App Server CPU Gauge */}
                <div className="mt-1.5 px-3">
                  <div className="flex justify-between text-[8px] text-zinc-500 font-mono mb-0.5">
                    <span>CPU:</span>
                    <span className={serverCpu > 40 ? "text-red-400 font-bold" : "text-zinc-400"}>{Math.round(serverCpu)}%</span>
                  </div>
                  <div className="w-full bg-zinc-950 h-1 rounded-full overflow-hidden border border-zinc-900">
                    <div 
                      className={`h-full transition-all duration-300 ${
                        serverCpu > 60 ? "bg-red-500" : serverCpu > 30 ? "bg-emerald-500" : "bg-zinc-700"
                      }`} 
          ...
          {/* Trigger Panel */}
          <div className="bg-zinc-900/30 border border-zinc-800/80 p-4 rounded-xl flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={sendRequest}
                disabled={isProcessing}
                className="py-2.5 px-5 rounded-xl bg-purple-655 hover:bg-purple-600 disabled:bg-zinc-850 disabled:text-zinc-600 font-semibold text-xs text-white transition-all flex items-center gap-2 shadow-lg shadow-purple-900/20 cursor-pointer disabled:cursor-not-allowed"
              >
          ...
          <div className="h-[150px] bg-zinc-950 rounded-xl border border-zinc-850 flex flex-col overflow-hidden">
            ...
            <div className="p-2.5 flex-1 overflow-y-auto space-y-1 font-mono text-[9.5px] select-text">
              {logs.length === 0 ? (
                <div className="text-zinc-650 text-center pt-10">
                  No logs recorded. Trigger an HTTPS request to start.
                </div>
              ) : (
                logs.map((log, index) => (
                  <div key={index} className="flex gap-1.5 text-zinc-400">
        ...
        <div className="w-full lg:w-[360px] shrink-0 flex flex-col gap-4 bg-zinc-900/30 border border-zinc-800/80 p-5 rounded-xl">
          ...
          {/* Strategy Details */}
          <div className="space-y-2.5 pb-3.5 border-b border-zinc-800">
            ...
            <div className="grid grid-cols-1 gap-1 text-[10px] pt-1">
              <div>
                <span className="font-bold text-green-400 block uppercase tracking-wider text-[8px] mb-0.5">Pros</span>
                <span className="text-zinc-300">{activeStrategyInfo.pros}</span>
              </div>
              <div className="mt-1">
                <span className="font-bold text-red-400 block uppercase tracking-wider text-[8px] mb-0.5">Cons</span>
                <span className="text-zinc-300">{activeStrategyInfo.cons}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-850 space-y-2">
            <div className="flex justify-between text-[10px] items-center">
              <span className="text-zinc-400">Internal Link Security:</span>
              {strategy === "termination" ? (
                <span className="text-red-400 font-bold flex items-center gap-1 text-[9px] bg-red-950/20 px-1.5 py-0.5 rounded border border-red-900/30">
                  <ShieldAlert className="w-3 h-3" /> Plaintext HTTP
                </span>
              ) : (
                <span className="text-emerald-400 font-bold flex items-center gap-1 text-[9px] bg-emerald-950/20 px-1.5 py-0.5 rounded border border-emerald-900/30">
                  <ShieldCheck className="w-3 h-3" /> Fully Encrypted
                </span>
              )}
            </div>
            <div className="flex justify-between text-[10px]">
              <span className="text-zinc-400">LB Overhead:</span>
              <span className="font-mono text-zinc-300">{activeStrategyInfo.lbCpu}</span>
            </div>
            <div className="flex justify-between text-[10px]">
              <span className="text-zinc-400">Server Overhead:</span>
              <span className="font-mono text-zinc-300">{activeStrategyInfo.serverCpu}</span>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col bg-zinc-950 rounded-xl border border-zinc-850 overflow-hidden min-h-[220px]">
            <div className="bg-zinc-900 px-3 py-1.5 border-b border-zinc-850 flex items-center justify-between">
              ...
              <span className="text-[8px] bg-purple-500/25 text-purple-300 px-1 rounded font-mono font-bold">
                conf
              </span>
            </div>
            <div className="p-3 font-mono text-[9px] text-zinc-400 overflow-x-auto whitespace-pre leading-relaxed select-text">
