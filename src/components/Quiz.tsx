...
export function Quiz({ questions }: QuizProps) {
    ...
    const [history, setHistory] = useState<{ selected: number; correct: boolean }[]>([]);
    ...
    const handleSubmit = () => {
        ...
        setHistory(prev => [...prev, { selected: selectedIdx, correct: isCorrect }]);
        ...
        return (
            <div className="my-8 p-6 rounded-2xl border border-border bg-card shadow-xl text-center w-full animate-in fade-in duration-300">
                ...
                <div className="space-y-4 mb-8 text-left">
                    <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Review Results</h4>
                    {questions.map((q, idx) => {
                            ...
                            <div key={idx} className="flex items-start gap-3 p-3 rounded-lg bg-muted/40 border border-border/50 text-sm">
                                ...
                                <div>
                                    ...
                                    <p className="text-muted-foreground mt-1 text-xs">
                                        Your answer: <span className="italic">{q.options[hist?.selected]}</span>
                                    </p>
                                </div>
                            </div>
                        );
        ...
        <div className=" my-8 p-6 rounded-2xl border border-border bg-card/60 backdrop-blur-md shadow-lg w-full animate-in fade-in duration-300">
            ...
            <div className="space-y-3 mb-6">
                {currentQuestion.options.map((option, idx) => {
                    ...
                    let optionStyles = "border-border hover:border-primary/50 hover:bg-muted/30";
                    if (isSelected) {
                        optionStyles = "border-primary bg-primary/5 dark:bg-primary/10";
                    }
                    if (showFeedback) {
                        if (isCorrect) {
                            optionStyles = "border-green-500/80 bg-green-500/10 text-green-900 dark:text-green-200";
                        } else if (isSelected) {
                            optionStyles = "border-red-500/80 bg-red-500/10 text-red-900 dark:text-red-200";
                        } else {
            ...
            <div className="space-y-4">
                ...
                <div className="flex justify-end">
                    {!showFeedback ? (
                        ...
                        >
                            {currentIdx < questions.length - 1 ? "Next Question" : "Show Results"}
