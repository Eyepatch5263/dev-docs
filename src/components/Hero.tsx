...
export function Hero() {
  ...
  return (
    <section className="relative min-h-screen lg:h-full w-full flex items-center justify-center overflow-visible lg:overflow-hidden px-4 sm:px-6 lg:px-8 bg-slate-50 dark:bg-slate-950 transition-colors duration-500">
        ...
        >
          ...
          <span className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 font-mono mt-0.5 flex items-center gap-1.5">
            <span className="relative flex h-1.5 w-1.5 md:h-2 md:w-2">
              <span
                className={`animate-ping absolute inline-flex h-full w-full rounded-full ${node.color} opacity-75`}
              ></span>
              <span
                className={`relative inline-flex rounded-full h-1.5 w-1.5 md:h-2 md:w-2 ${node.color}`}
              ></span>
            </span>
            {node.status}
          </span>
      ...
      {mounted && (
        <div
          className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none w-0 h-0 overflow-visible select-none block scale-[0.45] sm:scale-75 md:scale-100"
          style={{ zIndex: 1 }}
        >
          <SolarSystemOrbits />
