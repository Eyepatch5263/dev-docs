...
import { Plus, PlayCircle, Copy, Link2, Users, ArrowRight, Search, SlidersHorizontal, MoreHorizontal, FileText, Edit3, Send, CheckCircle, Globe, MessageSquare, GitFork, Shield } from 'lucide-react';
import Link from 'next/link';

const workflowSteps = [
    { icon: Users, label: 'Collaborate', description: 'Invite others and work together in real-time.' },
    { icon: GitFork, label: 'Fork', description: 'Fork the document to save a copy and make changes.' },
    { icon: Send, label: 'Submit', description: 'Only the original creator can submit for review.' },
    { icon: Shield, label: 'Admin Review', description: 'Admins review and approve before publishing.' },
    { icon: Globe, label: 'Published', description: 'Once approved, it goes live for everyone.' },
];

const documents = [
    { title: 'Database Indexing Explained', edited: 'Edited just now', collaborators: 2, time: '2 min ago', by: 'Arjun', status: 'DRAFT' },
    { title: 'CAP Theorem in Depth', edited: 'Edited 1 hour ago', collaborators: 1, time: '1 hour ago', by: 'You', status: 'SUBMITTED' },
    { title: 'Consistency Models', edited: 'Edited yesterday', collaborators: 0, time: 'Yesterday', by: 'Sarthak', status: 'APPROVED' },
    { title: 'Load Balancer Patterns', edited: 'Edited 2 days ago', collaborators: 3, time: '2 days ago', by: 'You', status: 'PUBLISHED' },
    { title: 'Caching Strategies', edited: 'Edited 3 days ago', collaborators: 0, time: '3 days ago', by: 'You', status: 'DRAFT' },
];

const activities = [
    { user: 'Arjun', action: 'made changes to', doc: 'Database Indexing Explained', time: 'Just now', icon: Edit3 },
    { user: 'You', action: 'submitted', doc: 'CAP Theorem in Depth', time: '1 hour ago', icon: Send },
    { user: 'Sarthak', action: 'commented on', doc: 'Consistency Models', time: 'Yesterday', icon: MessageSquare },
    { user: 'Kunal', action: 'forked', doc: 'Load Balancer Patterns', time: '2 days ago', icon: GitFork },
    { user: 'Admin', action: 'approved', doc: 'Consistency Models', time: '2 days ago', icon: CheckCircle },
];

const statusStyle: Record<string, string> = {
    DRAFT: 'border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-400',
    SUBMITTED: 'border-slate-400 dark:border-slate-600 text-slate-600 dark:text-slate-300',
    APPROVED: 'border-slate-400 dark:border-slate-500 text-slate-700 dark:text-slate-200',
    PUBLISHED: 'border-slate-500 dark:border-slate-400 text-slate-800 dark:text-slate-100',
};
            ...
            <div className="relative z-10 w-full h-full mx-auto px-8 lg:px-16 xl:px-24 flex flex-col justify-between py-10 lg:py-12 overflow-hidden">
                ...
                >
                    ...
                    <div className="flex flex-col justify-center">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-5 max-w-md">
                            Collaborate in real-time, fork to experiment, and publish only the best knowledge.
                        </p>
                            ...
                            {workflowSteps.map((step, i) => {
                                ...
                                return (
                                    <div key={step.label} className="flex flex-col items-center text-center flex-1 relative">
                                        ...
                                        <span className="text-[10px] font-semibold text-slate-700 dark:text-slate-300 mb-0.5">{step.label}</span>
                                        <span className="text-[8px] text-slate-400 dark:text-slate-500 leading-tight hidden lg:block max-w-[80px]">{step.description}</span>
                                    </div>
                                );
                            })}
                        </div>
                ...
                >
                    ...
                    <div className="lg:col-span-3 bg-white/60 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/50 rounded-xl p-4">
                        <h3 className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Start collaborating</h3>
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mb-3">Share a link with others to collaborate on a document in real-time.</p>
                        <div className="flex items-center gap-2">
                            <div className="flex-1 flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80">
                                ...
                                <span className="text-[11px] font-mono text-slate-400 dark:text-slate-500 truncate">https://explainbytes.tech/doc/abc123</span>
                            </div>
                    ...
                    <div className="lg:col-span-2 bg-white/60 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/50 rounded-xl p-4 flex flex-col justify-center">
                        <h3 className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-3">Live Collaborators</h3>
                        <div className="flex items-center gap-3">
                ...
                >
                    ...
                    <div className="lg:col-span-3 bg-white/60 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/50 rounded-xl p-4 flex flex-col overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-xs font-semibold text-slate-700 dark:text-slate-300">Your Documents</h3>
                            <div className="flex items-center gap-2">
                                <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80">
                                    ...
                                    <span className="text-[10px] text-slate-400">Search documents...</span>
                                </div>
                        ...
                        <div className="flex gap-4 mb-3 border-b border-slate-100 dark:border-slate-800/50 pb-2">
                            {['All', 'Owned by me', 'Shared with me', 'Forked by me'].map((tab, i) => (
                                <span
                                    key={tab}
                        ...
                        <div className="flex flex-col divide-y divide-slate-100 dark:divide-slate-800/30 flex-1 overflow-hidden">
                            {documents.map((doc) => (
                                <div key={doc.title} className="grid grid-cols-12 gap-2 items-center py-2.5">
                                    <div className="col-span-4 flex items-center gap-2">
                                        <FileText className="w-3.5 h-3.5 text-slate-300 dark:text-slate-700 shrink-0" />
                                        <div>
                                            <span className="text-[11px] font-medium text-slate-700 dark:text-slate-300 block leading-tight">{doc.title}</span>
                                    ...
                                    <div className="col-span-2">
                                        <span className={`inline-block px-2 py-0.5 rounded text-[8px] font-mono font-bold uppercase tracking-wider border ${statusStyle[doc.status]}`}>
                                            {doc.status}
                                        </span>
                                    </div>
                    ...
                    <div className="lg:col-span-2 bg-white/60 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/50 rounded-xl p-4 flex flex-col overflow-hidden">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-xs font-semibold text-slate-700 dark:text-slate-300">Recent Activity</h3>
                            <span className="text-[10px] text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 cursor-pointer transition-colors">View all</span>
                        </div>
                                    ...
                                    <div key={i} className="flex items-start gap-2.5">
                                        ...
                                        <div className="min-w-0">
                                            <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight">
                                                <span className="font-semibold text-slate-700 dark:text-slate-200">{item.user}</span>
                                                {' '}{item.action}{' '}
                                                <span className="font-medium text-slate-600 dark:text-slate-300">{item.doc}</span>
                                            </p>
                                            <span className="text-[9px] text-slate-400 dark:text-slate-500">{item.time}</span>
                                        </div>
                                    </div>
                                );
                            })}
                ...
                >
                    ...
                    Only document owners can submit for review and publish. Admins ensure quality before it goes live.
                </motion.div>
            </div>
        </section>
    );
