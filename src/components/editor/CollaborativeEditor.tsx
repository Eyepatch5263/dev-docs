...
function TiptapEditorInner({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "tiptap prose prose-lg dark:prose-invert max-w-none focus:outline-none min-h-[500px] px-8 py-6",
        spellcheck: "false",
      },
    },
...
export function CollaborativeEditor({
  ...
  useEffect(() => {
    ...
    prov.ws?.addEventListener("close", (event: CloseEvent) => {
      if (event.code === 4000) {
        setRoomFullError(
          event.reason || "Room is full. Maximum collaborators reached.",
        );
      }
    });
  ...
  if (roomFullError) {
    return (
      <div className="flex flex-col items-center justify-center h-[500px] text-center p-8">
        ...
        <p className="text-muted-foreground mb-6 max-w-md">{roomFullError}</p>
        <p className="text-sm text-muted-foreground mb-6">
          This document has reached its maximum number of collaborators. Please
          try again later or create a new document.
        </p>
