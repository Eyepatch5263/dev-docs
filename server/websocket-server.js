...
const MAX_CONNECTIONS_PER_ROOM = 2; // Maximum concurrent connections per document
...
wss.on("connection", (conn, req) => {
  ...
  console.log(
    `[${new Date().toISOString()}] Client attempting to connect to: ${docName}`,
  );

  // Check if room is full before allowing connection
  if (isRoomFull(docName)) {
    console.log(
      `[${new Date().toISOString()}] Room ${docName} is full (max ${MAX_CONNECTIONS_PER_ROOM}). Rejecting connection.`,
    );
    conn.close(
      4000,
      `Room is full. Maximum ${MAX_CONNECTIONS_PER_ROOM} collaborators allowed.`,
    );
    return;
  }
  ...
  console.log(
    `[${new Date().toISOString()}] Room ${docName} now has ${conns.size}/${MAX_CONNECTIONS_PER_ROOM} connections`,
  );
  ...
  conn.on("close", () => {
    console.log(
      `[${new Date().toISOString()}] Client disconnected from: ${docName}`,
    );

    // First, remove awareness states for this connection
    const controlledIds = conns.get(conn);
    console.log(
      `[${new Date().toISOString()}] Controlled IDs to remove:`,
      controlledIds ? Array.from(controlledIds) : "none",
    );

    if (controlledIds && controlledIds.size > 0) {
      const idsToRemove = Array.from(controlledIds);
      console.log(
        `[${new Date().toISOString()}] Removing awareness states for:`,
        idsToRemove,
      );
    ...
    if (orphanedIds.length > 0) {
      console.log(
        `[${new Date().toISOString()}] Cleaning up orphaned awareness states:`,
        orphanedIds,
      );
    ...
    console.log(
      `[${new Date().toISOString()}] Remaining awareness states after cleanup:`,
      remainingStates.size,
    );
  });
